...
buttonGeneratePaswword.addEventListener("click", () => {
  ...
  // slice the password to the amount of length specified
  const slicedpass = generatedPassword.slice(0, length);
  outputPass.innerHTML = slicedpass;
});
//event listener to copy password to clipboard
clipboardtxt.addEventListener("click", () => {
  const password = outputPass.innerText;
  if (!password) {
      swal ("opps!","Cannot copy blank password","error");
    return;
  }
  ...
  textArea.remove();
  swal("success","Password copied to clipboard","success");
});
